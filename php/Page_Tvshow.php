...
<?php
     ...
     *
     */
	$stmt=$conn->query("SELECT DISTINCT A.CATEGORY_ID, B.CATEGORY_NAME FROM VIDEO A JOIN CATEGORY B ON A.CATEGORY_ID=B.CATEGORY_ID where a.KIND_ID=2");
	 /*
     * 搜尋所有種類為綜藝節目的影片
    ...
    $count_tvshow = search_function($search,$category_,$kind_,null) -> fetchAll();
    /*
     * 計算有幾筆資料
     *
     */
    $all_num = count($count_tvshow);
    /*
     * 如果總數不被整除 , 頁碼要加一
     *
     */
	...
	/*
     * 頁碼設定
     * $now_pages == 1 && $all_num == 1 這個 if 判斷在 tvshow 資料多了之後可以拿掉
     *
     */
...
<body>
    <div id="main">
        ...
        <div id="context">
			...
			<table>
				<tr>
					<td colspan='2'>名稱</td><td>類別</td><td>類型</td><td>語言</td><td>開播日期</td><td>分數</td>
				</tr>
				<?php
					echo $table;
				?>
